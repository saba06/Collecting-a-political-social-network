...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
    id = set(users[2]['friends']).intersection(users[3]['friends'])
    request = robust_request(twitter,'users/lookup',{'user_id':id.pop()})
    for r in request:
        name = r['screen_name']
    return name
...
def create_graph(users, friend_counts):
    ...
    graph.add_nodes_from(f)
    for u in users:
        graph.add_edges_from([(u['screen_name'],id) for id in set(f).intersection(u['friends'])])
    return graph
